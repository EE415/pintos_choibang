  ...
  while (read_bytes > 0 || zero_bytes > 0) 
    {
      ...
      if(!add_exefile_sp(file, ofs, upage, page_read_bytes, page_zero_bytes, writable,false))
	return false;
      ofs += page_read_bytes;
...
setup_stack (void **esp) 
{
  ...
  kpage = frame_allocate((uint8_t *)PHYS_BASE - PGSIZE, false, true);
  if (kpage != NULL) 
    {
      ...
      {
        *esp = PHYS_BASE;
        thread_current()->stack_lim = ((uint8_t *) PHYS_BASE) - PGSIZE;
      }
      else
        palloc_free_page (kpage);
    }
